...
export default function BulkUploadFileStep({ marketType, file, fileError, rowCount, splitParts, currentPartNumber, totalParts, onFileSelect, onRemoveFile, onDownloadTemplate, onContinue }: Readonly<Props>) {
    const marketLabel = marketType === MarketType.SECOND_HAND ? "B2C Retail" : "B2B Wholesale";
        ...
        <div className="rounded-2xl border border-stroke-light bg-white p-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Upload a CSV or Excel file for the {marketLabel} bulk flow. Templates include the required platform headers for this product type.</p>
                </div>
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-1 text-xs text-muted-foreground">
                                    {formatFileSize(file.size)} • {rowCount} {rowCount === 1 ? "product row" : "product rows"}
                                </p>
                            </div>
                            <button
                                type="button"
                        ...
                        <div key={part.filename} className="rounded-xl border border-stroke-light bg-slate-50 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        {part.parsed.rowCount} {part.parsed.rowCount === 1 ? "product row" : "product rows"}
                                    </p>
                                </div>
                            </div>
                        </div>
